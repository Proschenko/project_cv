            ...
            for blur_ksize in blur_ksize_list:
                for block_size in block_size_list:
                    for threshold_type in threshold_types:
                        print(f"Итерация {iteration}: blur_ksize={blur_ksize}, threshold_type={threshold_type}, block_size={block_size}")
                        contours = detect_table(gray, blur_ksize, threshold_type, block_size)
                        table_image = extract_table(image, contours)

                        if table_image is not None:
                            output_path = f"{output_path_base}_iteration_{iteration}.jpg"
                ...
                if found:
                    break
            if not found:
                print(f"Не удалось найти таблицу на изображении после {iteration} итераций")
                break
...
output_path_base = f'signatures/{tmp}'
iterations = 4
